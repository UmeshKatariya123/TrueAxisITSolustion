...
const Logo = ({ className = "", showText = true, size = "default", variant = "light", useImage = false, imageSrc = null }) => {
  ...
  useEffect(() => {
    ...
    fetchLogo();
  }, [variant, useImage, imageSrc]);

  const getImageSrc = () => {
    if (useImage && imageSrc) return imageSrc;
    if (!logoData) return null;
    return logoData.imageType === 'base64' 
      ? `data:image/${logoData.imageFormat};base64,${logoData.imageData}`
      : logoData.imageData;
  };

  const imageSrcValue = getImageSrc();

  if (!loading && imageSrcValue) {
    return (
      <Link to="/" className={`flex items-center space-x-3 ${className}`}>
        <div className="flex-shrink-0">
          <img
            src={imageSrcValue}
            alt={logoData?.title || "TrueAxis IT Solutions Logo"}
            width={width}
            height={height}
      ...
      {showText && (
        <div className="flex flex-col">
          <div className={`font-bold ${textSize} leading-tight`}>
            <span className={textColors.trueText}>True</span>
            <span className={textColors.axisText}>
              Axis
            </span>
          </div>
          <div className={`text-xs ${textColors.tagline} font-semibold tracking-wider uppercase`}>
            IT Solutions
          </div>
        </div>
      )}
